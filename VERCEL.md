...
Project → Settings → Environment Variables → Add → сохранить → передеплой.

При запросах на объекты («подбери квартиру», «есть варианты» и т.п.) бот сам ходит в DuckDuckGo HTML с запросом вида «аренда недвижимости москва + запрос пользователя» и подставляет найденные варианты в ответ — без дополнительных API-ключей.

Без MINIMAX_API_KEY в чате будет сообщение об ошибке; виджет и интерфейс работают в любом случае.
